...
| 檔案 | 內容 | 需要音源? |
|---|---|---|
| `songs.js` | 340 首,闖關模式和競速「全部」的題目 | 要 |
| `themes.js` | 從 Apple Music 歌單匯入的 1,733 首,競速的其他主題用(見下面「主題」) | 自帶 |
...
競速模式開局前先選主題。「全部」就是原本的競速:只出 `songs.js`,套曲風配額。
其他主題從 `songs.js` + `themes.js` 合起來的曲庫裡篩,不套配額。每個主題的最佳成績分開記。

| 主題 | 怎麼篩 |
|---|---|
| 中文 · 台語 · 西洋 · 韓文 · 日文 | `genre` |
| 抖音歌 | 中文歌裡有標籤 `tiktok` 的(Douyin Music 熱歌榜 + 經典抖音神曲清單) |
| 動漫 · 影視 | 標籤 `ost`(動畫、偶像劇、韓劇歌單) |
| 經典老歌 · 2000 年代 · 2010 年代 | 華語和台語裡,依 `year` |

主題表在 `index.html` 的 `THEMES`。曲庫少於 30 首的主題不會出現在選單上。

**答案只從每個主題熱門度前 20% 的歌抽**(至少 30 首),其他歌只當九宮格選項 ——
